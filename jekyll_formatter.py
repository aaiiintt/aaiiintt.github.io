...
class JekyllFormatter:
    ...
    def create_post_file(self, content_data: Dict[str, Any], 
        ...
        # Use provided title or topic as fallback
        post_title = title or content_data["topic"]
        
        # Generate slug from the actual title
        from slugify import slugify
        slug = slugify(post_title, max_length=60)
        
        date_str = datetime.now().strftime("%Y-%m-%d")
        filename = f"{date_str}-{slug}.md"
        
        # Create front matter
        front_matter = self.create_front_matter(content_data, post_title, category)
